...
| <span style="white-space: nowrap;">구분</span> | 컬럼 | Pipeline 처리 |
|---|---|---|
| <span style="white-space: nowrap;">수치형</span> | `age`, `education_num`, `capital_gain`, `capital_loss`, `hours_per_week` | 중앙값으로 결측 대체 후 `StandardScaler` |
| <span style="white-space: nowrap;">범주형</span> | `workclass`, `education`, `marital_status`, `occupation`, `relationship`, `race`, `sex`, `native_country` | 최빈값으로 결측 대체 후 `OneHotEncoder` |
| <span style="white-space: nowrap;">제외</span> | `fnlwgt` | 개인 특성이 아닌 표본 가중치이므로 모델 입력에서 제외 |
