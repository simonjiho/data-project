...
def exclude_capital_outliers(
    ...
    print("\n[4. 자본손익 IQR 이상치 제외]")
    print(
        "capital_gain 양수값 유지 범위: "
        f"{information['gain_lower']:,.1f}~{information['gain_upper']:,.1f}"
    )
    print(
        "capital_loss 양수값 유지 범위: "
        f"{information['loss_lower']:,.1f}~{information['loss_upper']:,.1f}"
    )
    print(
...
def create_seaborn_chart(frame: pd.DataFrame, output_path: Path) -> None:
    """소득 그룹별 나이 분포와 상위 교육수준을 Seaborn Figure로 저장"""
    sns.set_theme(style="whitegrid")
    configure_korean_font()
    top_education = frame["education"].value_counts().head(8).index
    education_data = frame.loc[frame["education"].isin(top_education)]
    ...
    axes[1].set_title("교육수준별 소득 그룹 인원 수")
    axes[1].set_xlabel("인원 수")
    axes[1].set_ylabel("교육수준")

    fig.suptitle("Adult Census Income 탐색적 데이터 분석", fontsize=17, fontweight="bold")
    fig.tight_layout(rect=(0, 0, 1, 0.95))
    fig.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"\n[5. Seaborn 정적 차트 저장]\n{output_path}")
...
def run_normality_analysis(
    ...
    """소득 그룹별 정규성을 Shapiro-Wilk 검정과 분포·Q-Q plot으로 진단"""
    groups = {
        ">50K": frame.loc[
    ...
    for column_index, (group_name, values) in enumerate(groups.items()):
        ...
        distribution_ax.axvline(
            ...
            linestyle="--",
            linewidth=1.8,
            label=f"{group_name} 평균 {values.mean():.2f}",
        )
    ...
    distribution_ax.set_title(
        "소득 그룹별 주당 근무시간 분포 비교",
        fontsize=15,
        fontweight="bold",
    )
    distribution_ax.set_xlabel("주당 근무시간")
    distribution_ax.set_ylabel("밀도")
    distribution_ax.set_xlim(0, 100)
    distribution_ax.legend(ncol=2)
...
def run_rank_sum_test(frame: pd.DataFrame) -> dict[str, float | str]:
    ...
    # CLES: 고소득 그룹에서 뽑은 값이 저소득 그룹 값보다 클 확률(동점은 0.5 처리)
    cles = float(result.statistic) / (len(high) * len(low))
    ...
    print("\n[9. Mann-Whitney U (Wilcoxon rank-sum) 검정]")
...
def run_chi_square_test(frame: pd.DataFrame) -> dict[str, float | str]:
    ...
    print("\n[11. 카이제곱 독립성 검정: education × income]")
    ...
    print(f"효과 크기 해석: 관계의 크기는 {effect}입니다.")
    return {
        "chi2": chi2,
        "p_value": p_value,
        "p_value_text": p_value_text,
...
def write_report(
    ...
    missing = frame.isna().sum()
    missing_text = missing[missing > 0].to_string() if missing.sum() else "결측치 없음"
    model_rows = "\n".join(
        "| "
    ...
    if "excluded_high_income_rate" in exclusion:
        exclusion_bias_text = (
            ...
            "위 비율 차이가 클수록 이후 기술통계·검정·모델 평가가 원본 모집단보다 "
            "고소득 극단값이 적은 쪽으로 치우칠 수 있음을 감안해 해석해야 합니다."
        )
    else:
        exclusion_bias_text = ""
    ...
    if len(rejected_groups) == len(normality):
        normality_context = "두 그룹 모두 정규성 가설이 기각됐지만 표본 수가 크므로"
    elif rejected_groups:
        normality_context = (
            f"{', '.join(rejected_groups)} 그룹에서 정규성 가설이 기각됐지만 표본 수가 크므로"
        )
    else:
...
이 데이터는 미국 Census 자료에서 추출된 Adult 데이터셋이며, 목표는 개인의 연소득 50,000달러 초과 여부를 예측하는 것입니다. 컬럼 정의와 범주는 [UCI Adult 데이터셋 설명](https://archive.ics.uci.edu/dataset/2/adult)을 기준으로 정리했습니다.

## 1. 프로젝트 개요

- 목표: 인구통계·직업 특성을 이용해 연소득 `>50K` 여부를 분류
- 데이터: UCI Adult Census Income
- 최종 분석 크기: {len(frame):,}행 × {frame.shape[1]}열 (자본손익 이상치 제외 후)
...
```text
Pandas 로딩·정제: {load_times['Pandas']:.6f}초
Polars 로딩·정제: {load_times['Polars']:.6f}초
```

위 시간은 순수한 파일 읽기 시간만이 아니라 로딩부터 위 1차 정제까지 걸린 전체 시간입니다. 두 구현의 1차 정제 결과는 모두 {int(exclusion['before_count']):,}행 × {frame.shape[1]}열로 같았습니다. 이후 EDA에서 확인한 자본손익 이상치 {int(exclusion['excluded_count']):,}행을 제외해 최종 분석에는 {len(frame):,}행을 사용했습니다. 한 번의 실행 시간만으로 라이브러리의 절대적인 우열을 단정할 수 없으며, 파일 크기, 자료형 추론, 캐시와 하드웨어에 따라 결과가 달라질 수 있습니다.

## 3. 데이터 품질과 기술통계

중복 행을 제거했으며, 남은 결측치는 모델 Pipeline 내부에서 훈련 데이터의 중앙값 또는 최빈값으로 처리했습니다.
...
Boxplot과 `Q1 - 1.5 × IQR`, `Q3 + 1.5 × IQR` 경계는 이상치를 확정하는 규칙이 아니라 점검할 후보를 찾는 기준입니다. `capital_gain`과 `capital_loss`는 값의 대부분이 0이라 전체 데이터로 계산하면 Q1과 Q3가 모두 0이 되고, 0이 아닌 정상 관측치까지 전부 이상치 후보가 됩니다. 따라서 0은 정상적인 구조적 값으로 보존하고, 양수 관측치만 따로 모아 IQR 경계를 다시 계산했습니다.
...
- `capital_gain` 양수값 유지 범위: {exclusion['gain_lower']:,.1f} 이상 {exclusion['gain_upper']:,.1f} 이하
- `capital_loss` 양수값 유지 범위: {exclusion['loss_lower']:,.1f} 이상 {exclusion['loss_upper']:,.1f} 이하
...
EDA의 Boxplot은 제외 전 데이터를 보여주며, 이후 기술통계·시각화·통계검정·ML 모델에는 이상치를 제외한 데이터를 사용했습니다. `age`, `education_num`, `hours_per_week`의 IQR 후보는 실제로 가능한 값이고 특히 근무시간은 후보 비율이 높아 일괄 제거하지 않았습니다.
...
결측치는 주로 범주형 변수에 존재하며 행 전체를 삭제하지 않고, 학습 데이터에서 계산한 최빈값으로 Pipeline 내부에서 대체했습니다.

### 범주형 변수 Count bar plot

![주요 범주형 변수 Count bar plot](categorical_countplots.png)

`workclass`, `marital_status`, `race`, `sex`의 각 범주에서 소득 그룹별 절대 건수를 비교했습니다. Count bar plot은 표본 규모를 보여주므로 전체적으로 표본이 많은 범주의 막대가 크게 나타납니다. 특정 범주가 고소득과 얼마나 강하게 관련되는지는 절대 건수만으로 단정하지 않고, 범주 내부의 고소득 비율이나 카이제곱 검정 및 Cramér's V를 함께 확인해야 합니다. 결측값도 하나의 `결측` 범주로 표시해 누락 규모가 시각화에서 사라지지 않도록 했습니다.
...
Pearson 상관계수는 선형 관계만 나타내며 인과관계를 증명하지 않습니다. `capital_gain`·`capital_loss`처럼 0이 대부분인 zero-inflated 변수와 순서형 코드인 `education_num`에는 선형 상관의 전제가 잘 맞지 않으므로, 순위 기반 Spearman 상관계수를 함께 계산해 결과가 크게 달라지지 않는지 확인했습니다. `education_num`은 별도의 연속형 측정값이 아니라 범주형 `education`을 순서형 숫자로 표현한 코드이므로, 상관계수도 근사적인 경향으로만 해석해야 합니다. `fnlwgt`는 표본 가중치이므로 상관분석과 모델 입력에서 제외했습니다.
...
전체 표본이 매우 크면 Shapiro-Wilk 검정이 사소한 일탈에도 민감하므로, 그룹별로 고정된 난수 시드의 최대 5,000개 표본을 검정했습니다. p-value와 함께 분포 그래프 및 Q-Q plot에서 점들이 기준 직선에서 벗어나는지도 확인했습니다. 두 그룹 모두 정규성을 만족하지 않더라도 그래프와 검정 결과를 그대로 기록했습니다.
...
- 확률적 우월성(CLES): {rank_sum['cles']:.4f}
- 순위 양분 상관계수: {rank_sum['rank_biserial']:.4f} (효과 크기: {rank_sum['effect']})
- 해석: {rank_sum['interpretation']}

이 데이터의 두 소득 그룹은 서로 짝지어진 표본이 아니므로 Wilcoxon signed-rank가 아니라, 독립표본용 Wilcoxon rank-sum과 동등한 Mann-Whitney U 검정을 사용했습니다. 이 검정은 정규성 검정이 아니라 두 그룹의 값 또는 순위 분포 차이를 확인하는 비모수 검정입니다.

{median_note}, 순위 양분 상관계수는 {rank_sum['rank_biserial']:.4f}입니다. 확률적 우월성 {rank_sum['cles']:.4f}는 `>50K` 그룹에서 무작위로 뽑은 사람이 `<=50K` 그룹에서 뽑은 사람보다 오래 일할 확률(동점은 절반 처리)이 약 {float(rank_sum['cles']) * 100:.0f}%라는 뜻입니다. 중앙값 하나만 같다고 전체 분포까지 같다는 뜻은 아니며, 분포 그래프에서 `>50K` 그룹이 상대적으로 긴 근무시간 구간에 더 많이 분포하는지 함께 확인할 수 있습니다.
...
{normality_context} Welch t-test 결과도 참고하되, 비모수 순위합 검정과 그래프를 함께 해석했습니다. 표본이 크면 작은 차이도 통계적으로 유의할 수 있으므로 p-value뿐 아니라 평균 차이의 신뢰구간과 Cohen's d 같은 효과 크기를 함께 확인해야 합니다.
...
- 효과 크기 해석: 관계의 크기는 {chi_square['effect']}입니다.

p-value는 관계가 검출되는지를, Cramér's V는 관계가 실질적으로 얼마나 큰지를 보여줍니다. 카이제곱 근사는 기대빈도가 5 미만인 셀이 많으면 부정확해질 수 있어 기대빈도를 함께 점검했습니다. Cramér's V의 크기 기준은 분할표의 구조 `min(행 수 - 1, 열 수 - 1) = {int(chi_square['min_dimension'])}`에 따라 달라지므로, 이를 반영한 Cohen 기준(작음 {chi_square['small_threshold']:.2f} / 중간 {chi_square['medium_threshold']:.2f} / 큼 {chi_square['large_threshold']:.2f} 이상)으로 해석했습니다. 고정 기준(0.3 이상 = 큼)을 그대로 적용하면 두 범주뿐인 `income`과의 관계 크기를 과대 해석할 수 있습니다.

## 5. ML Pipeline 평가

- 비교 모델: `LogisticRegression`, `RandomForestClassifier`, `HistGradientBoostingClassifier`
- 최종 선택 모델: `{best_model_name}` (`ROC-AUC` 최고 모델)
- 목표값: 연소득 `>50K`이면 1, `<=50K`이면 0
- 데이터 분할: 훈련 80% / 평가 20%, 계층적 분할(`stratify`) 적용
- 평가 데이터 출처: 별도 파일을 내려받지 않고 동일한 `adult.data`에서 분리한 hold-out 20%. UCI의 `adult.test`는 사용하지 않음
- 평가 데이터 양성(`>50K`) 비율: {evaluation['test_positive_rate']:.1%} → 모두 `<=50K`로만 예측해도 정확도 {evaluation['majority_accuracy']:.4f}가 나오는 불균형 데이터이므로, Accuracy는 이 기준선과 비교해 해석
- 수치형 입력: `age`, `education_num`, `capital_gain`, `capital_loss`, `hours_per_week`
- 범주형 입력: `workclass`, `education`, `marital_status`, `occupation`, `relationship`, `race`, `sex`, `native_country`
- 제외한 컬럼: `fnlwgt`는 개인 특성이 아니라 표본 가중치이므로 모델 입력에서 제외
- 수치형 전처리: 중앙값 결측 대체 후 `StandardScaler`
- 범주형 전처리: 최빈값 결측 대체 후 `OneHotEncoder`
- 재현성: `random_state={RANDOM_STATE}`

| 모델 | Accuracy | F1 | ROC-AUC | PR-AUC | 학습 시간 |
|---|---:|---:|---:|---:|---:|
{model_rows}
...
Logistic Regression은 해석 가능한 선형 기준 모델, Random Forest는 여러 결정트리의 평균으로 비선형 관계와 과적합을 조절하는 앙상블, HistGradientBoosting은 이전 트리의 오류를 순차적으로 보완하는 부스팅 모델입니다. 세 모델에 동일한 데이터 분할과 전처리를 적용했으며, ROC-AUC가 가장 높은 `{best_model_name}`을 최종 Pipeline으로 저장했습니다. 결측 대체·스케일링·One-Hot Encoding은 Pipeline 안에서 훈련 데이터에만 학습되도록 구성했습니다.

양성 클래스가 소수인 불균형 데이터에서는 ROC-AUC가 실제보다 낙관적으로 보일 수 있어, 양성 클래스 기준의 PR-AUC(Average Precision)를 함께 보고했습니다. 또한 Random Forest에만 `class_weight="balanced_subsample"`을 적용했으므로 기본 임계값 0.5에 의존하는 Accuracy·F1은 세 모델 간 완전히 동일한 조건의 비교가 아니며, 임계값과 무관한 ROC-AUC·PR-AUC 중심으로 비교하는 것이 공정합니다.

## 6. 생성 산출물

- `adult_clean.parquet`: 중복 제거 후 정제 데이터
- `missing_values.png`: 컬럼별 결측 건수와 비율
- `numeric_boxplots.png`: 수치형 변수의 사분위수·이상치 후보 Boxplot
- `capital_outlier_comparison.png`: 자본손익 이상치 제외 전후 Boxplot
- `categorical_countplots.png`: 주요 범주형 변수와 소득 그룹의 Count bar plot
- `normality_diagnostics.png`: 소득 그룹별 분포와 Q-Q plot
- `seaborn_income_analysis.png`: 정적 EDA 차트
- `correlation_heatmap.png`: 수치형 변수 상관계수 히트맵
- `plotly_occupation_income.html`: 인터랙티브 직업별 고소득 비율
- `adult_income_pipeline.joblib`: 전처리와 ROC-AUC 최고 모델을 포함한 Pipeline
- `roc_curve.png`: 분류 모델의 ROC 곡선과 AUC
- `model_metrics.png`: 정확도·F1·ROC-AUC 비교 막대그래프
...
이번 분석은 단일 학습·평가 분할 결과입니다. 향후 교차검증, 하이퍼파라미터 탐색, 불균형 처리와 threshold 조정을 수행하고 Accuracy뿐 아니라 Precision·Recall 및 그룹별 성능을 함께 비교할 필요가 있습니다. 또한 `fnlwgt`를 단순 특성으로 사용하지 않았지만, 모집단 수준의 통계 추정에는 표본 가중치로 활용하는 별도 분석이 필요합니다.

방법론 측면의 한계로, 자본손익 이상치 제외 경계를 훈련·평가 분할 이전의 전체 데이터에서 계산했기 때문에 평가 데이터의 정보가 필터링 기준에 미세하게 섞였고, 모델 지표도 극단값이 제거된 데이터 기준이라 원본 분포에서의 성능보다 낙관적일 수 있습니다. 향후에는 극단 자본손익을 행 삭제 대신 winsorizing이나 로그 변환으로 완화하고, 제외가 꼭 필요하다면 그 경계를 훈련 데이터에서만 계산해 적용하는 방식이 통계적으로 더 안전합니다.
"""
    output_path.write_text(report, encoding="utf-8")
    print(f"\n[13. Markdown 보고서 자동 생성]\n{output_path}")
...
def run_pipeline(data_path: Path, output_dir: Path) -> None:
    ...
    spearman = analysis_frame[NUMERIC_COLUMNS].corr(method="spearman")
    analysis_frame.to_parquet(output_dir / "adult_clean.parquet", index=False)
    create_seaborn_chart(analysis_frame, output_dir / "seaborn_income_analysis.png")
