...
def write_report(
    ...
    correlation_table = dataframe_to_markdown(correlation, "변수", decimals=3)
    spearman_table = dataframe_to_markdown(spearman, "변수", decimals=3)
...
전체 표본이 매우 크면 Shapiro-Wilk 검정이 사소한 일탈에도 민감하므로, 고정된 난수 시드로 그룹별 최대 5,000개 표본을 추출해 검정했습니다. p-value와 함께 Boxplot의 중앙값·사분위 범위·이상치 후보 및 Q-Q plot에서 점들이 기준 직선에서 벗어나는지도 확인했습니다. 정규성 충족 여부와 무관하게 그래프와 검정 결과를 그대로 기록했습니다.
...
{median_note}, 순위 양분 상관계수는 {rank_sum['rank_biserial']:.4f}입니다. 위의 확률적 우월성 값은 `>50K` 그룹에서 무작위로 뽑은 사람이 `<=50K` 그룹에서 뽑은 사람보다 오래 일할 확률(동점은 절반 처리)을 뜻하며, 여기서는 약 {float(rank_sum['cles']) * 100:.0f}%입니다. 중앙값 하나만 같다고 전체 분포까지 같다는 뜻은 아니며, 정규성 진단 그림의 Boxplot에서 두 그룹의 분포 위치 차이를 함께 확인할 수 있습니다.
...
- 비교 모델: `Logistic Regression`, `Random Forest`, `HistGradientBoosting` (각각 sklearn `LogisticRegression`, `RandomForestClassifier`, `HistGradientBoostingClassifier`)
...
- 평가 데이터 양성(`>50K`) 비율: {evaluation['test_positive_rate']:.1%} → 모두 `<=50K`로만 예측해도 Accuracy {evaluation['majority_accuracy']:.4f}가 나오는 불균형 데이터이므로, Accuracy는 이 기준선과 비교해 해석
...
- 제외한 컬럼: `fnlwgt` (표본 가중치 — 제외 근거는 2장 용어 정리 표 참고)
- 수치형 전처리: 중앙값 결측 대체 후 `StandardScaler`
- 범주형 전처리: 최빈값 결측 대체 후 `OneHotEncoder`
- 불균형 처리: `Random Forest`에만 `class_weight="balanced_subsample"` 적용, 나머지 두 모델은 기본 설정
